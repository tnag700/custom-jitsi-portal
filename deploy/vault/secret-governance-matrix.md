...
| config-set encryption key | Security owner + backend owner | `kv/app/backend/runtime` | `backend-vault-bootstrap` -> bounded backend runtime bridge | planned cryptographic migration, compromise suspicion, recovery from tested backup | coordinated data re-encryption followed by controlled backend restart; never rotate as an ordinary secret | Vault path, private wrapped handoff, bounded runtime bridge `0600`; never repo-kept env files |
