...
describe("admin secondary navigation runtime", () => {
  it("buildAdminSecondaryHref preserves environment and queue context for role-history", () => {
    const currentUrl = new URL("https://portal.example.test/admin/incidents?environment=prod&view=critical");

    expect(buildAdminSecondaryHref(currentUrl, "/admin/role-history", "prod")).toBe(
  ...
  it("buildAdminSecondaryHref keeps detail-scoped identifiers for role-history drill-through", () => {
    ...
    expect(buildAdminSecondaryHref(currentUrl, "/admin/role-history", "test")).toBe(
  ...
  it("buildAdminSecondaryHref keeps config-sets bounded to environment and safe triage return path", () => {
    ...
    expect(buildAdminSecondaryHref(currentUrl, "/admin/config-sets", "dev")).toBe(
  ...
  it("resolveIncidentReturnTo falls back to queue when a secondary module has no explicit returnTo", () => {
    const currentUrl = new URL("https://portal.example.test/admin/role-history?environment=dev&subjectId=sub-1");

    expect(resolveIncidentReturnTo(currentUrl, "dev")).toBe("/admin/incidents?environment=dev&subjectId=sub-1");
  });
});