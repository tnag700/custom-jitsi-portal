...
export default component$(() => {
  ...
  const triageReturnHref = resolveIncidentReturnTo(
    location.url,
    filters.environment,
  );
  ...
  return (
    <div class="space-y-6">
      <section class="rounded-3xl border border-border bg-surface px-6 py-5 shadow-sm">
        <nav
          aria-label="Контекст разбора"
          class="flex flex-wrap items-center gap-2 text-sm text-muted"
        >
          <a href={overviewHref} class="underline">
            Сводка
          </a>
          <span>/</span>
          <a href={triageReturnHref} class="underline">
            Инциденты
          </a>
          <span>/</span>
        ...
        <a
          href={triageReturnHref}
          class="mt-3 inline-block text-sm text-muted underline"
        >
          Вернуться к очереди инцидентов
        </a>
        <p class="mt-2 text-xs uppercase tracking-[0.22em] text-muted">
          Вторичный модуль
        </p>
        ...
        >
          ...
          <label class="flex flex-col gap-1 text-sm text-muted">
            Поиск пользователя
            <input
              class="rounded-2xl border border-border bg-bg px-3 py-2 text-text"
              type="text"
              name="q"
              value={filters.q}
              placeholder="имя или ID субъекта"
            />
          </label>
          <label class="flex flex-col gap-1 text-sm text-muted">
            ID субъекта
          ...
          <label class="flex flex-col gap-1 text-sm text-muted">
            ...
            <input
              ...
              placeholder="assign/update/unassign"
            />
          </label>
          <label class="flex flex-col gap-1 text-sm text-muted">
            Роль
          ...
          <div class="sm:col-span-2 xl:col-span-3 flex gap-2">
            <button
              type="submit"
              class="rounded-2xl border border-text bg-text px-4 py-2 text-sm font-medium text-bg"
            >
              Показать историю
            </button>
            <a
      ...
      {!hasPrimaryFilter ? (
        <RequestStatePanel
          ...
          detail="Укажите поиск, ID субъекта, ID комнаты или ID встречи, чтобы загрузить ограниченную историю ролей и назначений."
        />
      ) : null}
        ...
        <section class="rounded-3xl border border-border bg-surface p-5 shadow-sm">
          <div class="flex items-center justify-between gap-3">
            <div>
              ...
              <p class="text-sm text-muted">
                тенант: {history.tenantId} · окружение: {history.environment} ·
                всего: {history.totalElements}
              </p>
            </div>
            <p class="text-sm text-muted">
              страница {history.page + 1} / {Math.max(history.totalPages, 1)}
            </p>
          </div>
          ...
          {history.content.length > 0 ? (
                ...
                >
                  <div class="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
                    <div>
                      ...
                      <p class="text-xs text-muted">
                        Оператор: {entry.actorLabel ?? "не указан"}
                  ...
                  <div class="mt-3 grid gap-1 text-xs text-muted sm:grid-cols-2 xl:grid-cols-4">
                    <p>
                      Роль: {entry.oldRole ?? "не задана"} →{" "}
                      {entry.newRole ?? "не задана"}
                    </p>
            ...
            <RequestStatePanel
              ...
              detail="Измените фильтры или временное окно."
            />
          )}
  ...
  meta: [
    {
      name: "description",
      content:
        "История ролей и назначений по пользователю, комнате и встрече внутри административной консоли.",
    },
  ],
};
