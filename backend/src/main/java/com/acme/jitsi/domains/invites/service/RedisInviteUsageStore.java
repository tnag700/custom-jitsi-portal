...
class RedisInviteUsageStore implements InviteUsageStore {
  ...
  public void assertCanConsume(InviteExchangeProperties.Invite invite) {
    ...
    try {
      ...
      long runtimeCount = currentRaw == null ? 0L : Long.parseLong(currentRaw);
      long current = runtimeCount + invite.usedCount();
  ...
  public void consume(InviteExchangeProperties.Invite invite) {
    ...
    try {
      ...
      long runtimeUsageCount = requireUsageCount(used);
      applyInitialExpiry(redisTemplate, key, invite, runtimeUsageCount);
      if (runtimeUsageCount + invite.usedCount() > invite.usageLimit()) {
        rollbackUsageCount(redisTemplate, key);
        throw new InviteExchangeException(HttpStatus.CONFLICT, ErrorCode.INVITE_EXHAUSTED.code(), "Лимит использований инвайта исчерпан.");
      }
    } catch (DataAccessException ex) {
      throw new RetryableInviteUsageException(
  ...
  private void applyInitialExpiry(
    ...
    if (runtimeUsageCount == 1L && invite.expiresAt() != null) {
  ...
  public void rollback(String inviteToken) {
    ...
    try {
      ...
      throw new RetryableInviteUsageException(
          HttpStatus.INTERNAL_SERVER_ERROR,
          ErrorCode.CONFIG_INCOMPATIBLE.code(),
          "Не удалось откатить резервирование инвайта.",
          ex);
    }
  }

  private void rollbackUsageCount(StringRedisTemplate redisTemplate, String key) {
